...
func fetchRentalListings(apiKey, city, state string) ([]RentalListing, error){
	url := fmt.Sprintf("https://api.rentcast.io/v1/listings/rental/long-term?city=%s&state=%S&status=Active&limit=20", city, state)

	// https://api.rentcast.io/v1/listings/rental/long-term?city=%s&state=%S&status=Active&limit=20
